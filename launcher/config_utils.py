...
def create_pipeline_variant_file(
    ...
    variant_path = os.path.join(variants_dir, "default.yaml")
...
def create_pipeline_exp_log_dir(
    ...
    status_log_file = os.path.join(log_dir, 'status.log')
    with open(status_log_file, "w") as log_file:
        log_file.write("waiting")
    is_exist = False
    return log_dir, is_exist
...
def load_config(exp_specs):
    config = Config()
    for key, value in exp_specs.items():

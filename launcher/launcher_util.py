...
def run_multi_processes(script_path, variant_paths, num_workers, gpu_id, log_paths):
    if len(variant_paths) < 0:
        return

    num_variants = len(variant_paths)
    num_workers = min(num_workers, num_variants)
    command = "python3 -m {script_path} -e {specs} -g {gpu_id}"
    ...
    try:
        while args_idx < num_variants or len(running_processes) > 0:
            if len(running_processes) < num_workers and args_idx < num_variants:
                ...
                status_path = os.path.join(log_paths[args_idx], "status.log")
                with open(status_path, "w") as f:
                    f.write("running")
                running_log_paths.append(log_paths[args_idx])
                args_idx += 1
            ...
            new_running_log_paths = []
            for d, p, l in zip(command_format_dicts, running_processes, running_log_paths):
                ret_code = p.poll()
                if ret_code is None or ret_code != 0:
                    new_running_processes.append(p)
                    new_command_format_dicts.append(d)
                    new_running_log_paths.append(l)
                    if ret_code is not None:
                        is_terminated = True
                elif ret_code == 0:
                    status_path = os.path.join(l, "status.log")
                    with open(status_path, "w") as f:
                        f.write("finished")
            running_processes = new_running_processes
            command_format_dicts = new_command_format_dicts
            running_log_paths = new_running_log_paths
    ...
    if is_terminated:
        for p in running_processes:
            p.terminate()
        for log_path in running_log_paths:
            status_path = os.path.join(log_path, "status.log")
            with open(status_path, "w") as f:
                f.write("running error")
        for idx in range(args_idx, num_variants):
            status_path = os.path.join(log_paths[idx], "status.log")
            with open(status_path, "w") as f:
                f.write("failed for previous error")

...
class CGANTrain(object):
    ...
    def cross_entropy_loss(self, predict, target):
        return torch.mean(-target.view(-1, 1) * torch.log(predict + self.eps) - (1 - target.view(-1, 1)) * torch.log(
            1 - predict + self.eps))

    def forward_batch(self, g_model, d_model, data_batch, mode, g_optimizer, d_optimizer):
        is_train = mode == 'train'
        res_list = []
        ...
        with torch.set_grad_enabled(is_train):
            ...
            real_loss = self.cross_entropy_loss(real_logits,
                                                torch.ones(category.size(0), dtype=torch.long).to(self.device))
            noise = torch.randn([category.size(0), self.config['noise_size'], self.config['noise_size']]).to(
                self.device)
            fake_imgs = g_model(noise, label)
            fake_logits = d_model(fake_imgs.detach(), label)
            d_fake_loss = self.cross_entropy_loss(fake_logits.float(),
                                                  torch.zeros(category.size(0), dtype=torch.long).to(self.device))
            ...
            for i in range(self.gen_dis_frequency_ratio):
                ...
                g_fake_loss = self.cross_entropy_loss(g_fake_logits.float(),
                                                      torch.ones(category.size(0), dtype=torch.long).to(self.device))
                if is_train:
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            for mode in self.modes:
                ...
                for bid, data_batch in enumerate(data_loaders[mode]):
                    ...
                    fake_imgs, d_loss, g_loss, gt_category, confidence_logits = self.forward_batch(generator,
                                                                                                   discriminator,
                                                                                                   data_batch,
                                                                                                   mode,
                                                                                                   g_optimizer,
                    ...
                    if mode == 'train' and self.step_counters[mode] % visual_freq == 0:
                        with torch.no_grad():
                            ...
                            for i in range(num_categories):
                                ...
                                noise = torch.randn([2, 1, self.config['noise_size'], self.config['noise_size']]).to(
                                    self.device)
                                fake_img = generator(noise, label)
                                img = fake_img[0][0].cpu().numpy().squeeze().reshape(224, 224) * 255
                                ax = fig.add_subplot(5, 5, i + 1)
                                ax.imshow(img, cmap="binary")
                            plt.savefig(os.path.join(log_dir, 'fake_imgs_epoch_{}_step_{}.png'.format(epoch,
                                                                                                      self.step_counters[
                                                                                                          mode])))
                            plt.close()
                pbar.close()

...
class QuickDrawVisualDataset(Dataset):
    ...
    def __getitem__(self, idx):
        if self.data is None:
            self.data = h5py.File(osp.join(self.root_dir, 'quickdraw_visual_{}.hdf5'.format(self.mode)), 'r')
        ...
        image = np.array(self.data[sketch_path][()], dtype=np.float32)
        sample = {'image': image, 'category': cid}
        return sample
